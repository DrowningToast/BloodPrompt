<script lang="ts">
    ...
    const saveDataHandler = async() => {
        await trpc.reward.update.mutate({data: {
           name : name_value || "",
           amount_left : parseInt(amount_left_value) || 0,
           description : description_value || "",
           required_points : parseInt(required_points_value) || 0,
           image_src: image_src_value || "" 
        }, rewardId: currentReward?.id || "1"}
		).then(()=>{alert("แก้ไขข้อมูลของรางวัลสำเร็จ");})
		.finally(()=>{goto("/staff/manage/reward")})
    }
	...
	<div class="flex flex-col justify-center items-center w-9/12 h-auto">
		...
		<div class="flex flex-row items-center justify-between px-14 h-32 w-full">
			...
			<div class="flex justify-between items-center gap-4">
				<Button
					on:click={saveDataHandler}
					class="flex justify-center gap-2 bg-[#EF4444] rounded-full text-center h-12 w-60 px-4 py-4 text-base font-bold text-white hover:bg-[#EF4444]"
					><Save class=" stroke-white w-5" />บันทึกข้อมูล</Button
				>
				<Button
				...
				<div class="flex flex-col gap-5 w-6/12">
					...
					<div class="flex flex-row justify-center items-center w-full gap-5">
						...
						<Button
							...
							on:click={() => (rewardImg = null)}>ลบรูปภาพ</Button
						>
					</div>
				</div>
				<div class="flex flex-col gap-5 w-6/12">
