<script lang="ts">
    ...
    import { Home, LogOut, CalendarDays, UserCircle, UserCircle2, KeyRound,MapPin, Lock, PlusCircle, Image, Info,Eye, Trash2 } from 'lucide-svelte';
    ...
    <div class="flex flex-col justify-center items-center w-9/12 h-auto">
        ...
        <div class="flex flex-row items-center justify-between px-14 h-32 w-full">
            <div class="flex flex-col">
                ...
                <p class="text-base text-gray-500">โปรดกรอกรายละเอียดเเละข้อมูลเกี่ยวกับสถานที่ใหม่ เพื่อทำการเพิ่มเข้าระบบ</p>
            </div>
            <div class="flex justify-between items-center gap-4">
                <Button class="flex justify-center gap-2 bg-[#EF4444] rounded-full text-center h-12 w-60 px-12 py-4 text-base font-bold text-white hover:bg-[#EF4444]"><PlusCircle class="fill-white stroke-[#EF4444]" />เพิ่มสถานที่ใหม่</Button>
        ...
        <div class="flex flex-col justify-center items-center gap-5 h-full w-full px-14">
            <div class="flex justify-between gap-20 bg-white w-full h-[581px] rounded-3xl shadow-2xl px-5 py-5">
                <div class="flex flex-col gap-5 w-6/12">
                    ...
                    <Select.Root multiple>
                        ...
                        <Select.Input name="selet" multiple/>
                    </Select.Root>

                    <div class="flex justify-start items-center gap-3">
                        <h1 class="font-bold">เปิดตั้งเเต่</h1>
                        <Input class="w-[166px] h-[50px] border-2 border-gray-300 px-4 py-4 rounded-xl" placeholder="เวลาเปิดทำการ"/>
                ...
                <div class="flex flex-col gap-5 w-6/12">
                    ...
                    <Input placeholder="ชื่อสถานที่รับบริจาคเลือด" class="rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4"/>
                    <Textarea placeholder="รายละเอียดเบื้องต้นของสถานที่" class="rounded-xl border-2 border-gray-300 h-[200px] w-full px-4 py-4 resize-none"/>
                    <Input placeholder="เบอร์โทรศัพท์" class="rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4"/>
                    <Input placeholder="เว็บไซต์ (ถ้ามี)" class="rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4"/>
                    <Input placeholder="อีเมลติดต่อ (ถ้ามี)" class="rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4"/>
                </div>
            </div>

            <div class="flex justify-between gap-20 bg-white w-full h-[382px] rounded-3xl shadow-2xl px-5 py-5">
                <div class="flex flex-col justify-between items-start gap-3 w-6/12 h-full pb-1">
                    <div class=" flex flex-row items-center gap-2">
                        <KeyRound class="w-6 stroke-2" />
                        <h1 class="font-bold py-2">รูปภาพประกอบของสถานที่</h1>
                    </div>
                    <Input placeholder="ชื่อ-นามสกุล" class="rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4"/>
                    <Input placeholder="อีเมล" class="rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4"/>
                    <div class="flex w-full justify-end items-center">
                        <Input type="password" placeholder="รหัสผ่าน" class="flex flex-row rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4 relative"/>
                        <Eye class="absolute transform -translate-x-4" />
                    </div>
                    <div class="flex w-full justify-end items-center">
                        <Input type="password" placeholder="ยืนยันรหัสผ่าน" class="flex flex-row rounded-xl border-2 border-gray-300 h-[50px] w-full px-4 py-4 relative"/>
                        <Eye class="absolute transform -translate-x-4" />
                    </div>
                    <div class="flex justify-center items-center w-full mt-1">
                        <Button class="px-14 py-3 rounded-full">เพิ่มบัญชี</Button>
                    </div>
                </div>
                    ...
                    <Table.Root class="bg-white rounded-3xl">
                        ...
                        <Table.Header>
                          <Table.Row class=" hover:bg-white">
                            ...
                            <Table.Head class="flex items-center justify-center font-bold text-[#EF4444]">ลบ</Table.Head>
                          </Table.Row>
                        </Table.Header>
                        <Table.Body>
                            ...
                            <Table.Row>
                                ...
                                <Table.Cell>จอร์ดี้ เอลนีโญ โปยา</Table.Cell>
                                <Table.Cell>Jordy.PH@gmail.com</Table.Cell>
                                
                                <Table.Cell class="flex flex-row justify-center items-center"> <Button class="flex justify-center items-center h-7 w-7 px-0 py-0 bg-[#EF4444]"><Trash2 class=" stroke-white h-5 w-5" /></Button> </Table.Cell>
                            </Table.Row>
                            {/each}
                        </Table.Body>
                    </Table.Root>
