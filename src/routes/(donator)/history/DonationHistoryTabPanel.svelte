<script lang="ts">
	...
	export let donationHistoryData: any;
</script>

<div class="flex flex-col gap-6">
	{#if donationHistoryData}
		{#each donationHistoryData as data}
			<!-- svelte-ignore a11y-click-events-have-key-events -->
			<!-- svelte-ignore a11y-no-static-element-interactions -->
			<div
				on:click={() => {
					goto('/history/donation/' + data.id);
				}}
			>
					...
					<Card.Content class="p-0 py-4">
						<div class="px-4 py-1 pb-0">
							<div class="flex flex-row items-center gap-1">
								<History size={18} />
								<p class="text-sm">
									ประวัติการบริจาคเลือดเลขที่: <span class="font-bold">{data.id.slice(-5)}</span>
								</p>
							</div>
							...
							<div class="mt-2">
								...
								<p class="font-bold text-sm flex flex-row items-center gap-1">
									...
									{:else if data.status === 'FAILED'}
										<p>บริจาคไม่สำเร็จ</p>
										<XCircle size={16} />
									{:else if data.status === 'WAIT_BLOOD_QUALITY'}
										<p>รอผลการบริจาค</p>
										<RefreshCcw size={16} />
									{/if}
								</p>
							...
							<div class="mt-2">
								...
								<p class="font-bold text-sm flex flex-row items-center gap-1">
									{#if data.blood_quality_status === 'QUALIFY'}
										<p>ผ่าน</p>
										<CheckCircle2 size={16} />
									{:else if data.blood_quality_status === 'DISQUALIFY'}
										<p>ไม่ผ่าน</p>
										<XCircle size={16} />
									{:else}
							...
							<div class="mt-2">
								...
								<p class="font-bold text-sm">{data.rewarded_points} แต้ม</p>
							</div>
							...
							<div class="flex flex-row items-center justify-around">
								<div class="flex flex-row items-center gap-1">
									<Calendar size={16} />
									<p class="font-semibold text-sm">{toDateString(data.created_at)}</p>
								</div>
